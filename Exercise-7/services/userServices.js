...
const registerUserService = async (userData) => {
  let response;
  try {
    const USER_DATA = await FILE_UTILS.readFile(USER_DATA_PATH);
    ...
    if (USER) {
      ...
      await FILE_UTILS.writeFile(USER_DATA_PATH, JSON.stringify(USER_DATA));
      const TASKS_DATA = await FILE_UTILS.readFile(TASK_DATA_PATH);
      TASKS_DATA.push({ username: userData.username, tasks: [], lastElementId: 0 });
      await FILE_UTILS.writeFile(TASK_DATA_PATH, JSON.stringify(TASKS_DATA));
      response = { status: true, message: "User Registered Successfully!!!", token: AUTH_UTILS.getToken({ username: userData.username }) };
    }
  } catch (err) {
...
const loginUserService = async (userData) => {
  ...
  try {
    ...
    if (USER) {
      ...
      if (IS_USER_VALID) {
        response = { status: true, message: "User logged in successfully!!!", token: AUTH_UTILS.getToken({ username: userData.username }) };
      } else {
